...
SignalR è un'astrazione di alcuni dei trasporti necessari per eseguire operazioni in tempo reale tra client e server. SignalR tenta innanzitutto di stabilire una connessione WebSocket, se possibile. WebSocket è il trasporto ottimale per SignalR perché contiene:
...
* Per i requisiti più severi, WebSocket richiede il server:
  * Eseguire in Windows Server 2012 o Windows 8.
  * .NET Framework 4.5

Se questi requisiti non vengono soddisfatti, SignalR tenta di utilizzare altri trasporti per stabilire le connessioni.
...
2. Se JSONP è configurato, ovvero se il `jsonp` parametro è impostato su `true` quando viene avviata la connessione, viene utilizzato il polling prolungato.
