...
1. Aprire Visual Studio 2013 con privilegi di amministratore. Selezionare **file**, **nuovo progetto**. Selezionare **Windows** sotto il nodo **Visual C#** nel riquadro **modelli** e selezionare il modello **applicazione console** . Assegnare al nuovo progetto il nome "SignalRSelfHost" e fare clic su **OK**.
...
5. Sostituire il contenuto del file Program.cs con il codice seguente.
    ...
    - **Avvio**, la classe che contiene la configurazione per il server SignalR (l'unica configurazione utilizzata in questa esercitazione è la chiamata a `UseCors` ) e la chiamata a `MapSignalR` , che consente di creare route per gli oggetti Hub nel progetto.
...
4. Fare clic con il pulsante destro del mouse sul progetto e scegliere **Aggiungi**, **nuovo elemento**. Selezionare il nodo **Web** e selezionare pagina HTML. Denominare la pagina **Default.html**.
