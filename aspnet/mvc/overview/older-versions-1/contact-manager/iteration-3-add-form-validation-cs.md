...
[Scarica il codice](iteration-3-add-form-validation-cs/_static/contactmanager_3_cs1.zip)
...
[![Finestra di dialogo relativa al nuovo progetto](iteration-3-add-form-validation-cs/_static/image1.jpg)](iteration-3-add-form-validation-cs/_static/image1.png)

**Figura 01**: Un modulo con convalida ([Fare clic per visualizzare l'immagine](iteration-3-add-form-validation-cs/_static/image2.png)a dimensioni intere )
...
Si noti, inoltre, le chiamate a Html.ValidationMessage() che vengono visualizzate accanto a ogni campo del form. L'helper ValidationMessage() visualizza un singolo messaggio di errore di convalida. Nel caso del listato 1, viene visualizzato un asterisco quando si verifica un errore di convalida.

Infine, l'helper Html.TextBox() esegue automaticamente il rendering di una classe di fogli di stile CSS quando si verifica un errore di convalida associato alla proprietà visualizzata dall'helper. L'helper Html.TextBox() esegue il rendering di una classe denominata **input-validation-error**.

Quando si crea una nuova ASP.NET applicazione MVC, nella cartella Content viene creato automaticamente un foglio di stile denominato Site.css. Questo foglio di stile contiene le seguenti definizioni per le classi CSS correlate all'aspetto dei messaggi di errore di convalida:
...
**Listato 2 - Controller-ContactController.cs (Crea con convalida)**
...
**Listato 3 - Controller-ContactController.cs**
