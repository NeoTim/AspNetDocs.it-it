...
Il primo &lt; riferimento allo script &gt; punta alla libreria di mapping di Microsoft Virtual Earth 6,2. Il secondo &lt; &gt; riferimento allo script punta a un file di map.js che presto creeremo che incapsula la logica di mapping JavaScript comune. L' &lt; elemento div id = "theMap" &gt; è il contenitore HTML che verrà usato da Virtual Earth per ospitare la mappa.

È quindi disponibile un &lt; &gt; blocco di script incorporato che contiene due funzioni JavaScript specifiche di questa visualizzazione. La prima funzione usa jQuery per collegare una funzione che viene eseguita quando la pagina è pronta per l'esecuzione di script sul lato client. Chiama una funzione helper LoadMap () che verrà definita all'interno del file di script Map.js per caricare il controllo mappa virtuale. La seconda funzione è un gestore eventi di callback che aggiunge un pin alla mappa che identifica una posizione.

Si noti come viene usato un &lt; blocco% =% sul lato server &gt; all'interno del blocco di script lato client per incorporare la latitudine e la longitudine della cena di cui si vuole eseguire il mapping nel codice JavaScript. Si tratta di una tecnica utile per restituire valori dinamici che possono essere usati da script lato client (senza richiedere una chiamata AJAX separata al server per recuperare i valori, rendendola più veloce). I &lt; blocchi% =% &gt; vengono eseguiti quando viene eseguito il rendering della visualizzazione nel server. l'output del codice HTML, ad esempio, è costituito da valori JavaScript incorporati, ad esempio var latitudine = 47,64312;).
...
Di seguito è riportato il codice JavaScript che verrà aggiunto al file di Map.js che interagisce con Virtual Earth per visualizzare la mappa e aggiungere i relativi punti per le cene:
...
>  [Avanti](enable-automated-unit-testing.md)
