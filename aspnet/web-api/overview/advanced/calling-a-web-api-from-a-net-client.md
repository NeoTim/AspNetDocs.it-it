...
**Nota:** Se si passano URL di base e URI relativi come valori hardcoded, tenere presenti le regole per l'utilizzo dell' `HttpClient` API. La `HttpClient.BaseAddress` proprietà deve essere impostata su un indirizzo con una barra finale ( `/` ). Ad esempio, quando si passano gli URI di risorsa hardcoded al `HttpClient.GetAsync` metodo, non includere una barra iniziali. Per ottenere un oggetto `Product` in base all'ID:

1. Set`client.BaseAddress = new Uri("https://localhost:5001/");`
1. Richiedere un `Product` . Ad esempio, `client.GetAsync<Product>("api/products/4");`
...
`RunAsync`esegue e si blocca fino a quando non viene completato. La maggior parte dei metodi **HttpClient** è asincrona perché eseguono I/O di rete. Tutte le attività asincrone vengono eseguite all'interno di `RunAsync` . Normalmente un'app non blocca il thread principale, ma questa app non consente alcuna interazione.
...
Questa classe corrisponde al modello di dati usato dall'API Web. Un'app può usare **HttpClient** per leggere un' `Product` istanza da una risposta http. L'app non deve scrivere codice di deserializzazione.
...
Il codice seguente invia una richiesta POST contenente un' `Product` istanza in formato JSON:
