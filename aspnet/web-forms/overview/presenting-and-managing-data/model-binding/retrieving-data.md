...
Nella cartella **Models** aggiungere una classe denominata **UniversityModels.cs**.
   ...
   3. Assegnare alla classe il nome **UniversityModels.cs** e selezionare **Aggiungi**.
      ...
      In questo file definire le `SchoolContext` classi, `Student` , `Enrollment` e `Course` come indicato di seguito:

      [!code-csharp[Main](retrieving-data/samples/sample4.cs)]

      La `SchoolContext` classe deriva da `DbContext` , che gestisce la connessione al database e le modifiche apportate ai dati.

      Nella `Student` classe si notino gli attributi applicati alle `FirstName` proprietà, `LastName` e `Year` . Questa esercitazione usa questi attributi per la convalida dei dati. Per semplificare il codice, solo queste proprietà sono contrassegnate con gli attributi di convalida dei dati. In un progetto reale, si applicano gli attributi di convalida a tutte le proprietà che richiedono la convalida.
...
Questa esercitazione USA [migrazioni Code First](https://docs.microsoft.com/ef/ef6/modeling/code-first/migrations/) per creare oggetti e tabelle di database. Queste tabelle archiviano informazioni sugli studenti e sui relativi corsi.
   ...
   2. Nella **console di gestione pacchetti**eseguire questo comando:  
      ...
      Si noti che è stato creato un file denominato *Configuration.cs* . La `Configuration` classe dispone di un `Seed` metodo, che può pre-popolare le tabelle di database con dati di test.

## <a name="pre-populate-the-database"></a>Popolamento preliminare del database

   1. Aprire Configuration.cs.
   
   2. Aggiungere il codice seguente al metodo `Seed` . Aggiungere inoltre un' `using` istruzione per lo `ContosoUniversityModelBinding. Models` spazio dei nomi.
   ...
   5. Eseguire il comando **Update-database**.
      ...
      Se si riceve un'eccezione quando si esegue questo comando, `StudentID` i `CourseID` valori e potrebbero essere diversi dai `Seed` valori del metodo. Aprire le tabelle di database e individuare i valori esistenti per `StudentID` e `CourseID` . Aggiungere tali valori al codice per il seeding della `Enrollments` tabella.
...
2. Individuare il `MainContent` segnaposto. All'interno di tale segnaposto, aggiungere un controllo **GridView** che includa questo codice.

   [!code-aspx-csharp[Main](retrieving-data/samples/sample6.aspx)]

   Aspetti da considerare:
   * Si noti il valore impostato per la `SelectMethod` proprietà nell'elemento GridView. Questo valore specifica il metodo utilizzato per recuperare i dati GridView, che vengono creati nel passaggio successivo. 
   
   * La `ItemType` proprietà viene impostata sulla `Student` classe creata in precedenza. Questa impostazione consente di fare riferimento alle proprietà della classe nel markup. Ad esempio, la `Student` classe ha una raccolta denominata `Enrollments` . È possibile usare `Item.Enrollments` per recuperare la raccolta e quindi usare la [sintassi LINQ](https://docs.microsoft.com/dotnet/csharp/programming-guide/concepts/linq/query-syntax-and-method-syntax-in-linq) per recuperare la somma dei crediti registrati di ogni studente.
...
## <a name="add-code-to-retrieve-data"></a>Aggiungere codice per recuperare i dati
   ...
   2. Aggiungere `using` istruzioni per gli `ContosoUniversityModelBinding. Models` `System.Data.Entity` spazi dei nomi e.
   ...
   3. Aggiungere il metodo specificato per `SelectMethod` :
      ...
      La `Include` clausola migliora le prestazioni delle query, ma non è obbligatoria. Senza la `Include` clausola, i dati vengono recuperati utilizzando il [*caricamento lazy*](https://en.wikipedia.org/wiki/Lazy_loading), che prevede l'invio di una query separata al database ogni volta che vengono recuperati i dati correlati. Con la `Include` clausola, i dati vengono recuperati utilizzando il *caricamento eager*, il che significa che una singola query di database recupera tutti i dati correlati. Se non si usano dati correlati, il caricamento eager è meno efficiente perché vengono recuperati più dati. Tuttavia, in questo caso, il caricamento eager offre le migliori prestazioni perché i dati correlati vengono visualizzati per ogni record.
...
Quando si imposta un valore per `SelectMethod` le `UpdateMethod` proprietà,, `InsertMethod` o `DeleteMethod` nel codice markup, è possibile selezionare l'opzione **Crea nuovo metodo** .
